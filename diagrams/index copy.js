...
async function renderMermaid (browser, definition, outputFormat, { viewport, backgroundColor = 'white', mermaidConfig = {}, myCSS, pdfFit } = {}) {
  ...
  try {
    ...
    if (outputFormat === 'svg') {
      ...
      console.log(viewport, clip);
      // await page.setViewport({ ...viewport, width: clip.x + clip.width, height: clip.y + clip.height })
      const pageSourceHTML = await page.content();
      // console.log(pageSourceHTML);
      // await fs.promises.writeFile(pageSourceHTML, outDefinition, 'utf-8')
      fs.writeFileSync('/Users/peteinge/Documents/index.html', pageSourceHTML);

      return {
        ...metadata,
        data: await page.screenshot({ viewport, omitBackground: backgroundColor === 'transparent' })
      }
    } else { // pdf
      if (pdfFit) {
        const clip = await page.$eval('svg', svg => {
  ...
  try {
    ...
    if (input && /\.(md|markdown)$/.test(input)) {
      ...
      for (const mermaidCodeblockMatch of definition.matchAll(mermaidChartsInMarkdownRegexGlobal)) {
        ...
        // console.log(outputFormat, parseMMDOptions);
